...
def compile_java_validator():
    if not pathlib.Path(
        "javavalidator/target/gfz-riesgos-xml-validator-1.0-SNAPSHOT.jar"
    ).exists():
...
def validate_java():
    ...
    invoke.run(
        f"java -jar javavalidator/target/gfz-riesgos-xml-validator-1.0-SNAPSHOT.jar {schema_file} {input_file}"
    )
...
def clean():
    for filename in [
        "example_out_shakemap_correlated.xml",
        "shakemap.xsd",
        "javavalidator/target/gfz-riesgos-xml-validator-1.0-SNAPSHOT.jar",
    ]:
